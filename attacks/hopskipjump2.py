...
class HopSkipJump():
    ...
    def generate(self, x, y=None, **kwargs):
        ...
        clip_min, clip_max = np.min(x,axis=0), np.max(x,axis=0)
    ...
    def _init_sample(
        ...
        if self.targeted:
            ...
            if all(adv_init!=None) and init_pred.cpu().numpy() == y_p: # achtung: init_pred ist tensor, y_p numpy
    ...
    def _binary_search(
        self,
        current_sample: np.ndarray, #[b, img]
        original_sample: np.ndarray,#[b, img]
        target: np.ndarray,#[b]
        norm,
        clip_min: float,#[b]
        clip_max: float,#[b]
        ...
        if norm == 2:
            ...
            (upper_bound, lower_bound) = (
                np.max(abs(original_sample - current_sample),axis=0),
                np.zeros(s)#0,
            )
        ...
        while np.any((upper_bound - lower_bound) > [threshold]*s):
            # Interpolation point
    ...
    def _compute_delta(
        ...
        if self.norm == 2:
            dist = np.linalg.norm((original_sample - current_sample),axis=0)
            delta = np.sqrt(np.prod(self._input_shape)) * self.theta * dist
        else: 
            dist = np.max(abs(original_sample - current_sample),axis=0)
